        ...
        <div v-if="album.length > 0">
            <div class="grid gird-flow-row grid-cols-3 lg:grid-cols-5 gap-5">
                <div v-for="(item, index) in album" :key="index">
                    <CoverItem :img-url="item.picUrl" @click="router.push({name: 'album', query: {id: item.id}})" />
                    <div class="mt-2 text-xs truncate">{{ item.name }}</div>
                    <div class="mt-1 text-xs text-slate-400 truncate">{{ moment(item.publishTime).format('YYYY-MM-DD') }}</div>
                </div>
            </div>
...
const getData = async() => {
    ...
    try {
        const data = await useArtistAlbum(props.id, pageData.limit, offset.value)
        if(pageData.page === 1) {
            album.value = data
        }else{
