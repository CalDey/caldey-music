<template>
    <div>
        <div class="safe-container">
            ...
            <div v-if="topListDetailData.length > 0" class="grid grid-flow-row grid-cols-1 lg:grid-cols-2 gap-5 cursor-pointer">
                <div v-for="item in topListDetailData.slice(0,4)" :key="item.id" class="bg-slate-100 card-radius flex flex-col md:flex-row">
                    <CoverItem :img-url="item.coverImgUrl" :play-count="item.playCount" show-play-count class="w-36 flex-shrink-0" />
                        ...
                        <div class="text-sm my-2">
                            <div v-for="(track, index) in item.tracks" :key="index">
                                <div class="flex my-1">
                                    ...
                                    <div class="truncate">{{ track.first }} - {{ track.second }}</div>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div v-else class="grid grid-flow-row grid-cols-1 lg:grid-cols-2 gap-5 cursor-pointer">
                <div v-for="(item, index) in 4" :key="index">
                    <el-skeleton animated>
                        <template #template>
                            <div class="flex">
                                <el-skeleton-item variant="image" style="width: 144px;height: 144px;" class="flex-shrink-0" />
        ...
        <div class="safe-container">
            ...
            <div v-if="topListDetailData.length > 0" class="grid grid-flow-row grid-cols-2 lg:grid-cols-4 gap-5">
                <div v-for="item in topListDetailData.slice(4)" :key="item.id">
                    <CoverItem :img-url="item.coverImgUrl" />
