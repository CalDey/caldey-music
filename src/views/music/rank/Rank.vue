<template>
    <div class="mb-20">
        <div class="safe-container">
            ...
            <div v-if="topListDetailData.length > 0" class="grid grid-flow-row grid-cols-1 lg:grid-cols-2 gap-5">
                <div v-for="item in topListDetailData.slice(0,4)" :key="item.id" class="bg-slate-100 card-radius flex">
                    <CoverItem :img-url="item.coverImgUrl" :play-count="item.playCount" show-play-count class="w-36 flex-shrink-0" @click="router.push({name: 'PlaylistDetail', query: {id: item.id}})" />
                        ...
                        <div class="text-sm my-2">
                            <div v-for="(track, index) in item.tracks" :key="index">
                                <div class="flex my-1">
                                    ...
                                    <div class="truncate" style="width: 30vw;">{{ track.first }} - {{ track.second }}</div>
                                </div>
                            </div>
                        </div>
                    </div>
