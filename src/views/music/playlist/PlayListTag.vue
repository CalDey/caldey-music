<template>
    <div v-if="tags" class="safe-container">
        <div class="grid grid-flow-row grid-cols-3 lg:grid-cols-6 gap-5">
            ...
            <div v-for="(tag, index) in tags" :key="tag.id" class="tag-container card-radius" :class="clickIndex === index ? 'bg-red-300 text-white' : 'bg-slate-200'" @click="tagClick(tag.name, index)">
                {{tag.name}}
            </div>
        </div>
    </div>
