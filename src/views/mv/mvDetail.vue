<template>
    <div class="safe-container flex flex-col lg:flex-row">
        <div class="w-full lg:w-2/3 border-0 lg:border mr-4 px-4 rounded-md">
            ...
            <div class="flex-1 py-4">
                <div v-if="video">
                    <div v-for="(item, index) in videoUrl" :key="index">
                        <video class="aspect-video w-full" :src="item?.url" controls/>
                    </div>
                </div>
                <video v-else class="aspect-video w-full" :src="mvUrl?.url" controls/>
            </div>
            <!-- 视频数据 -->
            <div>
                <div class="flex items-center">
                    <div v-if="mvDetail" class="flex">
                        <icon-svg class="w-8 h-8 mr-2 flex-shrink-0" icon-name="#icon-MV"></icon-svg>
                ...
                <div class="mt-2 text-xs text-gray-400">
                    <div v-if="mvDetail">
                        <span class="mr-2">发布：{{mvDetail.publishTime}}</span>
                        <span>播放次数：{{mvDetail.playCount}}</span>
                    </div>
                    <div v-else-if="videoDetail">
                        <span class="mr-2">发布：{{moment(videoDetail.publishTime).format('YYYY-MM-DD')}}</span>
                ...
                <div class="mt-2 text-sm">
                    <div v-if="mvDetail && mvDetailInfo" class="flex">
                        <!-- 喜欢 -->
                        <div class="flex items-center px-4 py-2 rounded-lg bg-gray-200 mr-4 cursor-pointer">
                            <icon-svg class="w-4 h-4" icon-name="#icon-like"></icon-svg>
                            <span>{{mvDetailInfo.likedCount}}</span>
                        </div>
                        <!-- 收藏 -->
                        <div class="flex items-center px-4 py-2 rounded-lg bg-gray-200 mr-4 cursor-pointer">
                            <icon-svg class="w-4 h-4" icon-name="#icon-star"></icon-svg>
                            <span>{{mvDetail.subCount}}</span>
                        </div>
                        <!-- 分享 -->
                        <div class="flex items-center px-4 py-2 rounded-lg bg-gray-200 cursor-pointer">
                            <icon-svg class="w-4 h-4" icon-name="#icon-share"></icon-svg>
            ...
            <div class="flex items-center mt-8 text-gray-500 text-sm pb-2 border-b">
                <icon-svg class="w-6 h-6 mr-2" icon-name="#icon-pinglun"></icon-svg>
            ...
            <div v-if="mvHotComments.length > 0">
                <div v-for="item in mvHotComments" :key="item.commentId" class="mt-2 flex">
            ...
            <div v-if="mvComments.length > 0" class="mb-2">
                <div v-for="item in mvComments" :key="item.commentId" class="mt-2 flex">
                    <el-avatar size="large" :src="item.user.avatarUrl" class="flex-shrink-0"></el-avatar>
                    <div class="ml-2">
                        <div>
                            <span>{{item.user.nickname}}</span>
        ...
        <div class="w-full lg:w-1/3 mb-20">
            ...
            <div class="mt-4 lg:mt-0 border p-4 rounded-md">
                ...
                <div v-if="mvDetail">
                    <span class="text-slate-600 text-sm">{{mvDetail.desc || '暂无内容'}}</span>
                </div>
                <div v-else-if="videoDetail" class="text-slate-600 text-sm">
                    <div class="flex items-center my-2">
                        <el-avatar class="flex-shrink-0 mr-2" shape="circle" :src="videoDetail.avatarUrl"></el-avatar>
            ...
            <div class="mt-4 border p-4 rounded-md">
                ...
                <div v-if="simiMv.length > 0">
                    <div v-for="item in simiMv" :key="item.id">
                        <CoverItem :img-url="item.cover" :play-count="item.playCount" show-play-count video class="mb-4" @click="router.push({name: 'mvDetail', query: {id: item.id, video: 'true'}})" />
                        <span class="flex items-center">
                            <icon-svg class="w-8 h-8 mr-2 flex-shrink-0" icon-name="#icon-MV"></icon-svg>
                            <span>{{item.name}}</span>
                        </span>
                        <div class="text-slate-400 text-sm mb-4">By.{{item.artistName}}</div>  
                    </div>
                </div>
                <div v-else-if="relatedVideo.length > 0">
                    <div v-for="item in relatedVideo" :key="item.vid">
                        <CoverItem :img-url="item.coverUrl" :play-count="item.playTime" show-play-count video class="mb-4" @click="router.push({name: 'mvDetail', query: {id: item.vid, video: 'true'}})" />
                        <span>{{item.title}}</span>
                        <div class="text-slate-400 text-sm mb-4">By.{{item.creator[0].userName}}</div>  
                    </div>
                </div>
            </div>
        </div>
...
import { useMvUrl, useMvDetail, useMvDetailInfo, useMvComments, useVideoUrl, useSimiMv,
         useVideoDetail, useVideoDetailInfo, useVideoComments, useRelatedVideo } from "@/utils/api";
import type { MvUrl, MvDetail, MvDetailInfo, MvHotComments, MvComments, SimiMv } from "@/models/mv";
...
const initVideoPage = async() => {
    ...
    if(route.query.video) {
        ...
        const { hotComments, comments, total } = await useVideoComments(id.value)
