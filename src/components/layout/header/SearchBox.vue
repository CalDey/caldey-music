<template>
    <div>
        ...
        >
            <el-input placeholder="请输入搜索关键字并按下回车键" v-model="keyword" @keydown.enter="search"></el-input>
            <div class="my-6">
                <span>热门搜索</span>
                <ul class="flex flex-wrap mt-2">
                    <li class="mr-2 bg-gray-200 p-1 my-1 rounded-md cursor-pointer hover:text-black" v-for="(item, index) in hotSearch" :key="index" @click="gotoSearchPage(item.first)">{{item.first}}</li>
                </ul>
            </div>
        </el-dialog>
    </div>
